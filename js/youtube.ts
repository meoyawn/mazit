...
export async function call(method: string, json: string): Promise<string> {
  try {
    return await operation(method, json);
  } catch (reason) {
...
async function operation(method: string, json: string): Promise<string> {
  ...
  if (method === "media") {
    ...
    if (
      selected.cipher ||
      selected.signature_cipher ||
      (selected.url && new URL(selected.url).searchParams.has("n"))
    ) {
    ...
    const clientConstants =
      Constants.CLIENTS[args.client as keyof typeof Constants.CLIENTS];
    const userAgent =
      clientConstants && "USER_AGENT" in clientConstants
        ? clientConstants.USER_AGENT
        : yt.session.context.client.userAgent;
    return JSON.stringify({
      url: url.toString(),
