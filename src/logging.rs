...
pub fn path() -> Result<PathBuf> {
    Ok(crate::engine::data_directory()?
        .join("logs")
        .join(CURRENT_FILE))
}
...
fn logger(directory: &Path, max_bytes: u64) -> Result<Logger> {
    ...
    Ok(
        Logger::try_with_str("off,mazit=info,gpui=warn,gpui_component=warn")?
            .log_to_file(
                FileSpec::default()
                    .directory(directory)
                    .basename("mazit")
...
pub fn init() -> Result<LoggerHandle> {
    let path = path()?;
    let handle = logger(
        path.parent().context("Log path has no directory")?,
        MAX_BYTES,
    )?
    .start()
    .context("Start file logging")?;
...
fn format(
    ...
    write!(
        ...
        record.level(),
        record.target(),
        crate::redact(&record.args().to_string())
    )
}
...
mod tests {
    ...
    fn logs_are_immediate_redacted_and_appended_across_restarts() {
        ...
        for _ in 0..2 {
            let (logger, handle) = logger(directory.path(), MAX_BYTES)
                .unwrap()
                .build()
                .unwrap();
            logger.log(
                &log::Record::builder()
                    ...
                    .args(format_args!(
                        "Completed https://example.com/audio?token=secret\nnext line"
                    ))
                    .build(),
            );
        ...
        for entry in 0..30 {
            logger.log(
                &log::Record::builder()
                    .target("mazit::engine")
                    .level(log::Level::Info)
                    .args(format_args!("Sync completed entry={entry:02}"))
                    .build(),
            );
        }
        handle.shutdown();
