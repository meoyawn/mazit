...
const version = '9.0.2';
const checksum = '8c3850283eb25fa026482078a04051e0be17347b09ef81a0849bec15a96e002e';
const archive = join(cache, `ffmpeg-${version}.tar.xz`);
const source = join(cache, `ffmpeg-${version}`);

if (
  await Bun.file(join(prefix, 'lib/libavformat.a')).exists() &&
  await Bun.file(join(prefix, 'COPYING.LGPLv2.1')).exists()
) {
  ...
  const flags = [
    ...
    '--enable-demuxer=mov,matroska,ogg,aac', '--enable-muxer=ipod',
    '--enable-decoder=aac,opus,vorbis', '--enable-encoder=aac',
    '--enable-parser=aac,opus,vorbis', '--enable-bsf=aac_adtstoasc',
    '--disable-x86asm',
  ];
